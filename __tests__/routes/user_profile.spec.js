...
describe('user_profile routes', () => {
  ...
  it('GET / should call getUserProfiles and return User Profile', async () => {
    controller.getUserProfiles.mockImplementation((req, res) => res.json(mockUserProfiles));

    const response = await request(app).get('/user_profile');

    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual(mockUserProfiles);
  ...
  it('POST / should call addUserProfile and return user email', async () => {
    ...
    controller.addUserProfile.mockImplementation((req, res) =>
      res.status(201).send(newUserProfile.email.toString())
    );
